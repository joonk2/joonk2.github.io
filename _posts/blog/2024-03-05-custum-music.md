...
<목차>
1. 음악은 어디서 구해오냐?
2. 음악 자동재생 기준으로 play, pause, next 버튼을 추가해보자
...
github나 gitlab에 올린 음악파일.wav 혹은 mp3는 용량이 커서 view raw를 누르면 다운로드 되는데 당황하지말고 일단 나의 글을 천천히 잘 읽어보자 <br>
...
참고로 `/_javascript/` 경로로 시도해본다면 안될 수도 있다. 내가 그 시행착오를 엄청 겪었다. <br>
만약 폴더명을 바꾸지 않고도 된다면 여러분은 행운이고, 그렇지 않다면 내 방법을 따라하여 나처럼 오랫동안 여기에 시간이 묶이지 않길 바랄 뿐이다.
